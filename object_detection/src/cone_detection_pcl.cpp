...
private:
    ...
    float distance_threshold)
    {
        ...
        for (const auto& indices : cluster_indices) {
            ...
            float base_diameter, average_intensity, height;
            if (isConeDetected(cluster, centroid, height, base_diameter, average_intensity)) {
                ...
                sensor_point.header.stamp = rclcpp::Time(0);
                ...
                try {
                    if (tf_buffer_->canTransform("map", "ZOE3/os_sensor", tf2::TimePointZero)) {
                        ...
                        for (const auto& waypoint : path_waypoints_) {
                            float distance = std::sqrt(std::pow(map_point.point.x - waypoint.first, 2) +
                                                       std::pow(map_point.point.y - waypoint.second, 2));
